...
class LlavaTopDownMetaForCausalLM(LlavaMetaForCausalLM):
    def encode_images_ps3(
        ...
        def _extract_vision_data(outputs):
            # PS3VisionModelOutput dataclass
            if hasattr(outputs, "last_hidden_state"):
                feats = outputs.last_hidden_state
                smaps = outputs.selection_maps
                sprobs = outputs.selection_probs
                # PS3VisionModelOutput.selection_maps/probs 是 tuple，要轉成 list 再用的話可在外面處理
                return feats, smaps, sprobs
            # 兼容 tuple/list
            if isinstance(outputs, (list, tuple)):
                f = outputs[0] if len(outputs) > 0 else None
                m = outputs[1] if len(outputs) > 1 else None
                p = outputs[2] if len(outputs) > 2 else None
                return f, m, p
        ...
        if only_select_first_n_scale is not None and num_look_close != 0:
            ...
            for x_mask in x_masks_each_group:
                ...
                image_features, selection_maps, select_probs = _extract_vision_data(v_out)
        ...
        if "mlp_downsample" in self.mm_projector.config.mm_projector_type:
            ...
            if self.use_high_res_pos_embed:
                low_res_h = low_res_w = int(low_res_image_features.shape[1] ** 0.5)
                pe = self.get_mm_projector().pos_embed_module.get_pe(low_res_h, low_res_w)
            ...
            if num_look_close == 0 or selection_maps is None:
            ...
            if self.use_high_res_pos_embed:
                pe_each_scale = []
                for i, smap in enumerate(selection_maps):
                    hi = ceil(smap.shape[-2] / downsample_ratio)
                    wi = ceil(smap.shape[-1] / downsample_ratio)
                    # pass selection_map so LogRetinaPosEmbed can compute dynamic center
                    pe = self.get_mm_projector().pos_embed_module.get_pe(hi, wi, selection_map=smap)
                    # pe: (1 or B, hi*wi, C)
                    pe = pe + self.get_mm_projector().high_res_scale_embed[i + 1]
                    pe_each_scale.append(pe)
            ...
            # add pos embed and scale embed to low-res tokens
            if self.use_high_res_pos_embed:
                lr_h = lr_w = int(low_res_image_features.shape[1] ** 0.5)
                pe = self.get_mm_projector().pos_embed_module.get_pe(lr_h, lr_w)
            ...
            if self.use_high_res_pos_embed:
                pe_each_scale = []
                for i, smap in enumerate(selection_maps):
                    pe = self.get_mm_projector().pos_embed_module.get_pe(
                        smap.shape[-2], smap.shape[-1], selection_map=smap
                    )
                    pe_each_scale.append(pe + self.get_mm_projector().high_res_scale_embed[i + 1])
                high_res_pos_emb_flatten = torch.cat(pe_each_scale, dim=1)
    ...
    def __embed_media_tokens(
        ...
        embeds = defaultdict(deque)
        top_down_selection_maps = None
        top_down_selection_probs = None
        for name in media:
            if self.training:
                # Gather metainfo of media objects from all ranks
                info = [{"shape": tensor.shape, "dtype": tensor.dtype} for tensor in media.get(name, [])]
    ...
    def generate_content(
        ...
        # --- 新增：解碼輸入端以進行 Debug ---
        # print("\n" + "="*20 + " DEBUG: INPUT DECODE " + "="*20)
        # # 解碼完整的輸入序列，查看 <image> 是否存在以及文字格式
        # input_decode = self.tokenizer.decode(input_ids[0], skip_special_tokens=False)
        # print(input_decode)
        # print("Input Token Count:", input_ids.shape[1])
        # print("="*60 + "\n")
        # ---------------------------------------
    ...
    def generate(
        ...
        **generation_kwargs,
        ...
        if media is not None:
            ...
            # print("look_close_mode: ", self.look_close_mode)
