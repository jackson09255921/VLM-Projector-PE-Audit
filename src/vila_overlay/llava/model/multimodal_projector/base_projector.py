...
from .positional_embeddings import build_pos_embed
...
class MultimodalProjector(PreTrainedModel):
    ...
    def __init__(self, mm_projector_cfg: MultimodalProjectorConfig, config: PretrainedConfig):
        ...
        if getattr(config, "ps3", False):
            ...
            if getattr(config, "high_res_pos_embed", False):
                # pos_embed_module replaces the old high_res_pos_embed nn.Parameter.
                # Ablation A ("learned") preserves the original parameter name inside
                # LearnedPosEmbed for easier checkpoint compatibility.
                self.pos_embed_module = build_pos_embed(config)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        """
        Backward-compatibility: remap old checkpoint key
            <prefix>high_res_pos_embed
        to new key
            <prefix>pos_embed_module.high_res_pos_embed
        before delegating to the standard loader.
        """
        old_key = prefix + "high_res_pos_embed"
        new_key = prefix + "pos_embed_module.high_res_pos_embed"
